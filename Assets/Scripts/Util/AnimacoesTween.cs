...
public class AnimacoesTween : MonoBehaviour {
	...
	public static void AnimarObjeto (GameObject ObjAnimar, 
		...
		switch(TipoAnimacao){
			...
			break;
			case TiposAnimacoes.RotacaoSubMenu:
				// Sequence tween = null;

                // tween = DOTween.Sequence()
                //     .Append( ObjAnimar.transform.DOLocalRotate( new Vector3( 0, 0, 15 ), TempoAnimacao ).OnComplete(() => tween.Pause() ) )
                //     .Append( ObjAnimar.transform.DOLocalRotate( new Vector3( 0, 0, -15 ), TempoAnimacao ).OnComplete( () => tween.Pause() ) )
                //     .SetLoops( -1 );
            	
            	// tween.Play();

				// ObjAnimar.transform.DOLocalRotate(new Vector3(0,0,15), TempoAnimacao).OnComplete(() =>
				// 	ObjAnimar.transform.DOLocalRotate(new Vector3(0,0,-15), TempoAnimacao)
